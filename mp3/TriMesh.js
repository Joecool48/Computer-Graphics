 ...
 */
    ...
    */
    computeAABB(){
        this.minXYZ[0] = this.vBuffer[0];
        this.maxXYZ[0] = this.vBuffer[0];
        this.minXYZ[1] = this.vBuffer[1];
        this.maxXYZ[1] = this.vBuffer[1];
        this.minXYZ[2] = this.vBuffer[2];
        this.maxXYZ[2] = this.vBuffer[2];
        this.centerXYZ[0] = 0;
        this.centerXYZ[1] = 0;
        this.centerXYZ[2] = 0;
        // compute the min and max bounding coords
        for (let i in this.vBuffer) {
            this.minXYZ[i % 3] = this.min(this.minXYZ[i % 3], this.vBuffer[i]);
            this.maxXYZ[i % 3] = this.max(this.maxXYZ[i % 3], this.vBuffer[i]);
            this.centerXYZ[i % 3] += this.vBuffer[i];
        }
        // compute the center number
        this.centerXYZ[0] /= this.numVertices;
        this.centerXYZ[1] /= this.numVertices;
        this.centerXYZ[2] /= this.numVertices;
    ...
    */
    loadFromOBJ(fileText)
    {
        ...
        for (let i in lines) {
            let line = lines[i].split(/\s{1,}/); // remove all spaces and split on them
            if (line.length !== 4) continue // not a valid face or vector
            else if (line[0] === "v") {
                // exract the xyz coords of the vertex
                this.vBuffer.push(parseFloat(line[1]));
                this.vBuffer.push(parseFloat(line[2]));
                this.vBuffer.push(parseFloat(line[3]));
                this.numVertices++;
            }
            else if (line[0] === "f") {
                // take the face and fill the fBuffer with the points in the vBuffer by their ids
                for (let i = 1; i <= 3; i++) {
                    let idx = parseInt(line[i]);
                    // push the vector associated with that index. Make sure to subtract 1
            ...
            // found a comment
                ...
                console.log("Comment on line " + i + 1 + ": " + lines[i]);
            }
            else {
                console.log("Found unexpected line beginning at line " + i + 1);
            }
    ...
    */
    loadBuffers()
    {
      console.log("There are " + this.fBuffer.length + " faces");
        // Specify the vertex coordinates
        this.VertexPositionBuffer = gl.createBuffer();
        gl.bindBuffer(gl.ARRAY_BUFFER, this.VertexPositionBuffer);
        gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(this.vBuffer), gl.DYNAMIC_DRAW);
